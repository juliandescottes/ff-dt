...
if [ -d devtools ]; then
  echo "devtools folder already exists. (It shoudn't exists)"
  exit
fi

echo "Cloning mozilla-central git repo, this takes quite some time (3GB to download)"
echo "Takes about 6 minutes with good bandwith"
time git clone --single-branch https://github.com/mozilla/gecko-dev devtools
